...
def load_model(model_path,):
    from video_data_generator import recall_m, precision_m, f1_m
    # load model here
    ...
    with strategy.scope():
        ...
        ts_model = tf.keras.models.load_model(
            model_path, custom_objects={'recall_m':recall_m, 
                                        'precision_m':precision_m, 
                                        'f1_m':f1_m}, compile=True, options=None
        )
...
def make_prediction(model, bs, infer_path):
    ...
    save_path = 'predictions/{}/'.format(infer_path.split('/')[-1])
